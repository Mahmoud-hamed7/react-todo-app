...
export default function TodoElement({ todo }) {
  ...
  return (
    <>
      ...
      {isDeleting && (
        <div className="confirmation-box shadow-lg p-4" style={{ borderRadius: "15px", position: "fixed", top: "50%", left: "50%", transform: "translate(-50%, -50%)", zIndex: 1001, background: "white", width: "90%", maxWidth: "400px" }}>
          <h4 className="fw-bold text-danger">حذف المهمة؟</h4>
          <p>هل أنت متأكد من حذف: <strong>{todo.title}</strong>؟</p>
          <div className="d-flex justify-content-end gap-2 mt-3">
             <button onClick={() => setIsDeleting(false)} className="btn btn-light">تراجع</button>
             <button onClick={confirmDelete} className="btn btn-danger px-4">حذف</button>
          </div>
        </div>
      )}
      ...
      {isUpdating && (
        <div className="confirmation-box shadow-lg p-4" style={{ borderRadius: "15px", position: "fixed", top: "50%", left: "50%", transform: "translate(-50%, -50%)", zIndex: 1001, background: "white", width: "90%", maxWidth: "400px" }}>
          <h4 className="fw-bold" style={{color: theme.primary}}>تعديل المهمة</h4>
          <input
          ...
          <textarea
            ...
            rows="3"
            onChange={(e) => setupDating({ ...upDating, details: e.target.value })}
            value={upDating.details}
          />
          <div className="d-flex justify-content-end gap-2">
            <button onClick={() => setIsUpdating(false)} className="btn btn-light">إلغاء</button>
            <button onClick={handelUpdating} className="btn text-white px-4" style={{backgroundColor: theme.primary}}>حفظ التعديل</button>
          </div>
        </div>
      )}
        ...
        <div className="card-body d-flex align-items-center justify-content-between p-3">
          ...
          <div className="d-flex gap-1">
            ...
            <button
              ...
              style={{ borderRadius: "50%", width: "40px", height: "40px", color: theme.primary }}
            >
              🖋️
            </button>
            ...
            <button
              ...
              className="btn btn-sm btn-light shadow-sm"
              style={{ borderRadius: "50%", width: "40px", height: "40px", color: theme.danger }}
            >
              🗑️
            </button>
          </div>
